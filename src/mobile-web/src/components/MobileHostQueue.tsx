...
export function MobileHostQueue({ queue, onRestore }: { queue: HostDialogQueue; onRestore: (text: string) => void }) {
  ...
  return <section className="host-message-queue" aria-label={t('queue.title')}>
    <div className="host-message-queue__header">
      <button type="button" className="host-message-queue__toggle" aria-expanded={expanded} aria-controls={listId}
      ...
      <MobileIconButton appearance="plain" size="sm" aria-label={t('queue.about')} aria-expanded={showHelp} aria-controls={helpId}
      ...
      <ul id={listId} hidden={!expanded} className="host-message-queue__list">
        {view.snapshot?.items.map(item => <li key={item.turnId}>
          ...
          <div className="host-message-queue__actions">
            ...
            <MobileIconButton appearance="plain" size="sm" aria-label={t('queue.cancel')} disabled={busy || !!view.error || item.status === 'steering_pending'} icon={<X size={18} aria-hidden="true" />} onClick={() => void run(() => queue.act(item, 'cancel'))} />
          </div>
        </li>)}
