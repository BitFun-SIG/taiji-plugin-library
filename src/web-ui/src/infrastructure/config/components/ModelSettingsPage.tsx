...
const ModelSettingsPage: React.FC = () => {
  ...
  const renderEditingForm = () => {
    ...
    const authSelectValue = authIsSubscription
      ...
      : 'api_key';
    const authOptions: ComboboxOption[] = SUBSCRIPTION_AUTH_OPTION_VALUES.map((value) => {
      if (value === 'api_key') {
      ...
      if (value === 'subscription:grok') {
        return { value, label: t('subscriptionAuth.options.grok') };
      }
      if (value === 'subscription:hermes') {
        return { value, label: t('subscriptionAuth.options.hermes') };
      }
                          ...
                          {account.provider === 'qoder' && (
                            <>
                              ...
                              <Button
                                size="sm"
                                variant="fill"
                                loading={patLoggingIn === account.provider}
                                disabled={anyLoginInProgress}
