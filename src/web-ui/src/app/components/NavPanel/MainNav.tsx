...
const MainNav: React.FC<MainNavProps> = ({
  ...
  const primaryAssistantWorkspaceId = assistantWorkspacesList[0]?.id ?? null;
  const defaultAssistantWorkspace =
    assistantWorkspacesList.find(w => w.id === primaryAssistantWorkspaceId)
  ...
  const handleGroupChatCreated = useCallback(async (groupId: string, name: string) => {
    ...
    // Group = agent_type="group" session; registering it locally into
    // flowChatStore lets the existing SessionScene open it.
