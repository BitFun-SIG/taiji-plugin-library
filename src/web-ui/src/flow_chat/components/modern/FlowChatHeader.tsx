...
import { Keyboard, Maximize2, Minimize2, Move, Square } from 'lucide-react';
...
export interface FlowChatHeaderProps {
  ...
  /** Session ID. */
  sessionId?: string;
  /** Latest user message text for tooltips and the drag payload. */
...
export const FlowChatHeader: React.FC<FlowChatHeaderProps> = ({
  ...
  const rightActions = (
    ...
    >
        ...
        {sessionId ? (
          <Tooltip content={t('flowChatHeader.dragToAuxiliary')}>
            <IconButton
        ...
        <Tooltip content={t(isChatFullWidth ? 'layout.fullWidth.exit' : 'layout.fullWidth.enter')}>
          <IconButton
            ...
            icon={isChatFullWidth ? <Minimize2 size={14} /> : <Maximize2 size={14} />}
          />
        </Tooltip>
  ...
  // Keep the header mounted for an active (possibly empty) session: the drag
  // handle and full-width toggle must stay reachable before the first turn.
