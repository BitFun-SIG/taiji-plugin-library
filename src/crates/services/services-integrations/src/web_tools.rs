...
/// Local-fork request payload for the MCP Exa search endpoint used by the
/// built-in web search tool. The upstream-free-exa path builds its body
/// directly from `WebSearchRequest`, but the tool-level provider keeps the
/// richer request shape (type / livecrawl / context budget).
#[derive(Debug, Clone)]
