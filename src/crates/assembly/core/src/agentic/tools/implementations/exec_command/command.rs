...
{description}"#
    ...
    /// Best-effort resolved working directory for the command guard: resolves
    /// the requested (or workspace default) workdir so edit-constraint checks
    /// see the directory the command actually targets. Unresolvable paths
    /// return `None`, which makes the guard fall back to workspace-root-only
    /// semantics instead of blocking the command.
    fn guard_workdir(input: &Value, context: &ToolUseContext) -> Option<String> {
        let raw = Self::requested_workdir(input, context).ok()?;
...
- In non-TTY mode, stdout and stderr ordering is not guaranteed; use tty=true or redirect stderr with 2>&1 when terminal ordering matters."#
    ...
    fn permission_intents(
        ...
        let _working_directory = Self::guard_workdir(input, context);
    ...
    async fn validate_input_rewrite_invariants(
        ...
        let Some((context, parsed)) = context
            .zip(exec_command_run_input_from_input(input))
            .filter(|(context, _)| {
